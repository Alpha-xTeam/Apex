...
- الإيجاز إلزامي: الـ story 2 جمل لا أكثر، الـ task جملة واحدة، الـ title 5-10 كلمات. لا تكرر نفسك.
- الـ title: عنوان عربي محدد وجذاب — ليس عاماً. كل تحدي يجب أن يكون قصة مختلفة جذرياً عن أي تحدي آخر.
- الـ story: سيناريو واقعي من 2 جمل يربط الـ title بسياق حقيقي.
- الـ task_outline: ماذا يجب على الطالب فعله، جملة واحدة بدون ذكر الأوامر بالأحرف.
...
- الـ hints: 3 تلميحات سقراطية قصيرة (جملة واحدة لكل تلميح) — توجّه الطالب للتفكير بدون إعطاء الجواب.
...
مهم جداً:
- لا تكتب أي نص قبل أو بعد الـ JSON. أخرج JSON فقط.
- لا تضف ```json أو ``` — JSON خام فقط.
- أبقِ الإجمالي أقل من 1200 كلمة لتفادي القطع.

أعد JSON حصراً بهذا الشكل (بدون أي نص خارجه):
{{
  "title": "عنوان عربي محدد وجذاب",
  "story": "قصة واقعية 2 جمل بدون أسماء خوارزميات",
...
# jargon (hash, salt, signature, ciphertext) — only the task gets sanitized.
_LEAK_REPLACEMENTS = {
...
def _sanitize_task_outline(text: str) -> str:
    ...
    (e.g. "Caesar" → "this shift cipher" must not produce "Cthis shift
    cipherar").
    """
    if not text:
        return text
    ...
    # before shorter prefixes (AES) — otherwise "AES" would shadow the
    # longer match.
